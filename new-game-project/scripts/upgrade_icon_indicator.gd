...
# For the little list of icons in round.
# Called when the node enters the scene tree for the first time.
func _refresh() -> void:
	for child in get_children():
		if child is Label:
			continue
		child.queue_free() # Get rid of the old ones though.
	# Create an icon for each upgrade.
...
func _start() -> void:
	_refresh()
	Globals.start_wave.connect(_refresh) # refresh each wave.
